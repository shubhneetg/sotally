...
test.describe('Settings Page', () => {
  // Test 50: Settings page renders all four sections (Profile, Password, API Keys, Notifications, Danger Zone)
  ...
  test('notification preference toggles can be clicked to toggle their state', async ({ page }) => {
    ...
    // Find the "Marketing" toggle which starts as off (false)
    ...
    // Find the Marketing switch (should be aria-checked="false" initially)
    const marketingSwitch = page.getByRole('switch').filter({ hasText: '' }).nth(2);
